...
def generate_article_quiz(text):
    """Generate a 3-question True/False quiz from the article text."""
    prompt = (
        "Based on the following article text, generate a 3-question True or False quiz to test reading comprehension. "
        "Provide the response in valid JSON format as a list of objects, where each object contains 'question' and 'answer' fields. "
        "The 'answer' should only be True or False. Do not include any additional text outside the JSON:\n\n"
        f"{text}\n\n"
    )
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a helpful assistant that generates True or False quizzes based on text."},
            {"role": "user", "content": prompt}
        ]
    )

    # Parse response into JSON
    print("Raw response from OpenAI (quiz):", response.choices[0].message.content.strip())

    raw_content = response.choices[0].message.content.strip()
    if raw_content.startswith("```json") and raw_content.endswith("```"):
        raw_content = raw_content[7:-3]  # Remove the ```json and ``` delimiters
    
    quiz_questions = json.loads(raw_content)
...
def summarize():
    try:
        ...
        title, content,top_image = fetch_article_content(article_url)
        ...
        quiz_questions = generate_article_quiz(content)
